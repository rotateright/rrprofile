...
static void pmc_setup_one_p4_counter(unsigned int ctr)
{
	...
	if (counter_config[ctr].event <= 0 || counter_config[ctr].event > NUM_EVENTS) {
		printk(KERN_ERR 
		       "rrprofile: P4 event code 0x%lx out of range\n",
	...
	printk(KERN_ERR 
	       "rrprofile: P4 event code 0x%lx no binding, stag %d ctr %d\n",
...
static void p4_setup_ctrs(struct op_msrs const * const msrs)
{
	...
	if (! MISC_PMC_ENABLED_P(low)) {
		printk(KERN_ERR "rrprofile: P4 PMC not available\n");
		return;
	}
...
#ifdef RRPROFILE
			oprofile_add_sample_start(end_timestamp);
			oprofile_add_sample_stop(oprofile_get_tb());
#endif // RRPROFILE
